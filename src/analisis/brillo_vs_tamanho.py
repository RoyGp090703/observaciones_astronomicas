...
# Normalización de la magnitud absoluta para la escala de brillo
...
# Generación del gráfico de dispersión con variables visuales
scatter = ax.scatter(
...
# Ajuste de ejes y formato logarítmico
...
# Anotación de los asteroides extremos en tamaño
extremos = pd.concat([df_limpio.nlargest(2, 'diameter_m'), df_limpio.nsmallest(2, 'diameter_m')])
for _, row in extremos.iterrows():
    desplazamiento = (-15, 0) if row['H_brillo_norm'] > 0.5 else (15, 0)
    alineacion = 'right' if row['H_brillo_norm'] > 0.5 else 'left'
    ax.annotate(
        row['name'], 
...
# Configuración de la barra de color personalizada
divider = make_axes_locatable(ax)
cax = divider.append_axes("bottom", size="4%", pad=0.05) 
cbar = plt.colorbar(scatter, cax=cax, orientation='horizontal')
altura_y = -1.8 
cbar.ax.text(0.0, altura_y, 'Menos brillante', transform=cbar.ax.transAxes, va='top', ha='left', fontsize=12, color='#4a0082')
cbar.ax.text(0.5, altura_y, 'Escala de brillo (H)', transform=cbar.ax.transAxes, va='top', ha='center', fontsize=12, color="#000000")
cbar.ax.text(1.0, altura_y, 'Más brillante', transform=cbar.ax.transAxes, va='top', ha='right', fontsize=12, color='#d4a000')
